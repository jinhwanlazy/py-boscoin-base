...
import mock
from boscoin_base.federation import *
...
class TestFederation(object):
    ...
    @raises(FederationError)
    @mock.patch('boscoin_base.federation.get_federation_service')
    def test_federation_none_service(self, get_service):
        get_service.return_value = None
        federation('fed*stellar.org')

    @mock.patch('boscoin_base.federation.requests.get', side_effect=mocked_requests_get)
    @mock.patch('boscoin_base.federation.get_federation_service')
